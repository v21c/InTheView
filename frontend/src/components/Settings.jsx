...
const Settings = ({ user, isOpen, onClose }) => {
  ...
  const [currentPassword, setCurrentPassword] = useState(""); // Changed from emailCurrentPassword and passwordCurrentPassword for consistency
  ...
  const handleEmailChange = async (e) => {
    e.preventDefault();
    try {
      await reauthenticateUser(currentPassword); // Reauthenticate the user before updating the email
  ...
  const handlePasswordChange = async (e) => {
    ...
    try {
      await reauthenticateUser(currentPassword); // Reauthenticate the user before updating the password
  ...
  const handleDeleteAccount = async () => {
    ...
    // Prompt user for current password to reauthenticate
    ...
    try {
      ...
      await axios.delete(`http://localhost:5000/api/users/${user.uid}`);
      console.log("Account deleted successfully");

      // Sign out the user and redirect to the login page or home page
      await auth.signOut();
